---
title: "Global Mortality Rates in Adults and Children"
author: "Natasha Gulati 1003202054"
date: "22/12/2020"
output:
...
## Keywords
Men, women, child, mortality rate, Log-Quad, SVD-Model

## Abstract

Using the Log-Quad and SVD Model within the linear regression models, we deduce and compare the mortality rates between men, women, and children in global contexts. Contrasting developing nations as well as low and high mortality rated countries, we gain a larger look into the meaning of the mortality rate.


## Introduction

  Mortality rates from around the world vary due to age, sex and region. There are a plethora of variables to account the mean age at which people pass and at what age. This report will be a reproduction and analysis on the mean age of adult and child mortality rates in developed and developing countries (Clarke, 2019). 

  Mortality rates for adults and children are important to discuss as the trends that are prominent in regions may result in a larger issues such as socio-economic status, health-care availability and possibly the genetic disposition of the area at hand. The variables being discussed will be the sex, year, age and region which will include Sweden, South Africa, Mexico and Austria. We will be using the SVD-Comp and the Log-Quad model to analyze the model in order to interpret the outcome of the data. The SVD-Comp model is a linear structure of the model which lacks the need to be altered in order to manipulate the data, and the weights will appropriately be distributed and is matrix based. The Log-Quad model is to get the age-specific mortality rates based on age-specific coefficients. Both will be used to derive conclusions from linear regression models. 


## Methodology and Models

We will be using the log-quadratic (Log-Quad) model and it has the form: $$\log(m_x) = a_x+b_xh+c_xh^2+v_xk$$ where x is the age-specific mortality, the coefficients a, b, and c are constants for the quadratic regression model. Then, h is the value for the log value, v is the correction factor where k is the respective coefficient. (Clarke, 2019). This model will help calculate the error of the residuals of the quadratic part of the data which is associated with the mortality rates of each variable which is not easily done with the other model used by matrix algorithms, the SVD-Comp.

The SVD-Comp is formulated as follows: $$\ m = \sum_i^cw_i*u_i+r$$ where m is the vector of mortality rates and the value c is the sets containing the observed mortality rates. Thus w is the weighted coefficients and r is the residuals in vector form (Clarke, 2019).

Together, they are used to gain a more in-depth age-specific mortality rate when it comes to men, women and children globally, comparing the error of residuals along the way with simple and quadratic linear regression.

The Human Mortality Database (HMD) has been accumulated from www.Mortality.org (www.mortality.org/hmd/zip/all_hmd/hmd_statistics.zip). This information has provided us with tables and information that provides us with countries from around the world and the mortality rates for the men, women and children. We will be focusing on the Mexican and South African mortality rates around the same years (1980's), and then Sweden and Austria's mortality rates with a significant 200 years difference. Lastly in figure 3, we will see the SVD-Comp linear regression model coefficients. 

Figure 1: Mexico and South African Mortality rates


```{r echo=FALSE, include=FALSE}

################################important######################################
#ALL THE CODE USED IS FROM https://github.com/sinafala/svd-comp/blob/master/Rmd/SVD-Comp.Rmd s
# SAM cLARK 2019 REPRODUCIBLE DATA
...
```{r packages-for-analysis, echo=FALSE, eval=TRUE, include=FALSE}
library(readr)
library(stringr)
library(reshape2)
library(ggplot2)
...
```{r clear, eval = TRUE, echo = FALSE, include=FALSE, results = 'hide'}
...
```{r download-functions, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r download-hmd, eval = TRUE, echo = FALSE, include=FALSE, results = 'hide'}
# try the download
...
```{r inspect-download-result, eval = FALSE, echo = FALSE, include=FALSE}
if (class(download.result)!="try-error") {
...
```{r parse-hmd-life tables, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r look-at-lists, eval = TRUE, echo = FALSE, include=FALSE}
list.files("../RData/")
```
...
```{r structure-of-lists, eval = TRUE, echo = FALSE, include=FALSE}
str(hmd.5x1.list, list.len = 4, vec.len = 2)
```


```{r structure-of-life-table, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r make-column-matrices, eval = TRUE, echo = FALSE, include=FALSE}
# 1x1 nqx
...
```{r look-at-matrices, eval = TRUE, echo = FALSE, include=FALSE}
list.files("../RData/")
```


```{r remove-balarus, eval = TRUE, echo = FALSE, include=FALSE}
## females
which(is.na(q1.f[1,]))
...
```{r remove-flat, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r remove-zeros, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r log-logit-transform, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r mx-indicators, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r clean-up, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r svdMod-function, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r ltPredict-function, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r svd-model-test, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r make-five-year-predicted, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r log-quad-comparison-function, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r, compare-SVD-LQ, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r run-50-50percent-samples, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r run-50-vary-sample-size, eval = TRUE, echo = FALSE, include=FALSE}
# female
adult <- FALSE
smooth <- FALSE
N <- 50
...
```{r plot-basic-age-relationships, eval = TRUE, echo = FALSE, include=FALSE}
# age relationships
# female
...
```{r plot-log-quad-comparison, eval = TRUE, echo = FALSE, include=FALSE}
...
Figure 2: 
```{r plot-example-data, eval = TRUE, echo = FALSE}
...
```{r plot-svd, eval = TRUE, echo = FALSE, include=FALSE}
# svds
...
```{r plot-single-year-errors, eval = TRUE, echo = FALSE, include=FALSE}
# female
...
```{r plot-sample-size-error-dists, eval = TRUE, echo = FALSE, include=FALSE}
# medians
# female
es.s.f <- rbind(
...
```{r plot-right-singular-vectors-by-child-mortality, eval = TRUE, echo = FALSE, include=FALSE}
# svds
svd.m <- mod.1_0.m$svd$s1
svd.f <- mod.1_0.f$svd$s1
...
```{r plot-adult-mortality-by-child-mortality, eval = TRUE, echo = FALSE, include=FALSE}
# data
am.cm <- rbind(
...
```{r plot-q0-by-child-mortality, eval = TRUE, echo = FALSE, include=FALSE}
# data
q0.cm.m <- data.frame(
...
```{r plot-heuristic-predicted, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r life-tables-used, eval = TRUE, echo = FALSE, include=FALSE, warning=FALSE}
...
```{r component-ss, eval=TRUE, echo=FALSE, include=FALSE}
...
```{r summary-comparison-table, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r tables-of-regression-models, eval = TRUE, echo = FALSE, include=FALSE, warning=FALSE}
...
```{r age-specific-error-comparisons, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r table-of-components, eval = TRUE, echo = FALSE, include=FALSE}
...
```{r age-specific-error-comparisons-various-components, eval = TRUE, echo = FALSE, include=FALSE}
# 1 component
...
```{r test-other-countries, eval = TRUE, echo = FALSE}
# Mexico
...
```{r make-mods-object, eval=TRUE, echo=FALSE}
...
```{r wrap-up, eval=TRUE, echo=FALSE, include=FALSE}
...
# SVD-Comp linear regression coefficients of Adult Mortality on Child Mortality derived from all models

Figure 3:

Disclaimer:
Coefficient abbreviations are as follows:

cm - child mortality, natural scale.
cml - child mortality, logged.
cmls - square of logged child mortality.
cmlc - cube of logged child mortality.
am - adult mortality, natural scale.
aml - adult mortality, logged.
amls - square of logged adult mortality.
amlc - cube of logged adult mortality.
...
Coefficients:
(Intercept)           cm          cml         cmls         cmlc  
    5.30956     -9.58727      3.65390      0.62664      0.04169  
...
Coefficients:
(Intercept)           cm          cml         cmls         cmlc  
   -1.12485      3.71753      0.11933      0.05888      0.01332  


## Results

In figure 1 we can see there is an obvious discrepancy in the manipulation of the data with the Log-Quad model and the SVD-Comp model for the evaluation in South Africa, especially for females. Otherwise, the SVP-Comp and Log-Quad model are evident to have similar predictions. The 'checkmark' or log trend are congruent in all graphs except for the South African female. For both countries, males have the greatest increase in mortality rate from ages 15-19 to around 30, where the trend then becomes relatively linear.

In figure 2, we have a comparison of a high mortality graph of Sweden in 1751 and Austria in 1990, though seemingly obscure to compare, there are plenty of notable aspects to this comparison. First, we see that the curve for Sweden in significantly higher than that for Austria's. Then we can see that the mortality rate for men and women in Sweden are highly similar, and the mortality rate between men and women in Austria are significantly different with men having a age-specific death rate than women at the time, that carries through all the way until the end of the curve.

In figure 3 we have the SVD-Comp model comparing the female and male age mortality rate in children and adults with weights of natural scale, logged, square and cubed of logged across all models. We use SVD-Comp in this case as it is specific to age and it deems similar to the simple linear regression model. For adult mortality on child mortality for females it is  -9.58727, so for every increase in adult mortality for females there is an inverse relationship with the child mortality rate, where it decreases by approximately 9. The intercept of 5.30956 tells us that the child mortality rate for females starts at approximately 5.For men, the coefficient is  3.71753, which means for every increase in adult mortality for males, child mortality increases by 3 in relation. The intercept -1.12485 is negative, so we cannot make a definite conclusion to the meaning of this.


## Discussion

For figure 1, we can make predictions for the evidence displayed. For the most part the values of the mortality rate can be similar, though looking closer to South Africa as an Under Developed country, we can come to many conclusions. Focusing on females in Africa, we know that there is a large under-service of medical health care and basic living establishments that cannot be met, along with diseases such as Sickle Cell (Gardner, 2018) that cannot be treated and run rampant genetically. Along with the physical composition, females are likely to die at a young age or birth due to males being more prized in society as well. Women are less likely to be able to be the bread winner and go to school, so therefore males surviving would be overall more beneficial for the families needs.In comparison to Mexico, South-African females have a higher mortality rate over all, and plateau around 45 years of age, whereas Mexican females have a steady increasing mortality rate from 10-14.We can reason that this may be due to South African women not having proper medical aid when delivering/labouring children. As well, the prevalence of AIDS being passed down genetically without supportive care severely impairs the health of men and women in South Africa can explain the clear peaks in the data.(Gardner, 2018)

Within figure 2 we can make the basic assumption that due to natural causes and the lack of modern medicine the average mortality rate for men and women were significantly higher compared to more than 200 years later. Both men and women have very similar mortality rates in the mid-1700's in comparison to 1990 where men are far higher to pass away younger than women. We cannot make any clear distinction to why this is true in terms of Austria, but we can make an educated guess that men have to do more laborious work and thus health and physical composition have taken a toll and therefore are more risk-prone.

Figure 3 establishes that for natural scale men and women, average values are in favour for men having a generally higher mortality rate than women. This is wildly known, and can conclude that this is a likely outcome for these numbers. 



## Next Steps

Within this report the lack of additional information of variables such as death cause and sample population is unknown. For example, the data does not account for the entire population and as such we cannot come to exact conclusions. Moving forward, if the cause of death for each person was categorized in natural occurrence (such as genetic or terminal disease) or an accident/other, we can make more intrinsic deductions of the meaning of these results. 

As well, possibly comparing more countries with clearly different socio-economic backgrounds would aid in discovering the possible differences within the countries to make more meaningful inferences. Or, possibly comparisons of multiple under-developed/developing nations to analyze perceived trends between them. 



# Reproducibility 

All of the tables, graphs and data used in this report was derived from Sam Clark located within his article and data : ‘A General Age-Specific Mortality Model With an Example Indexed by Child Mortality or Both Child and Adult Mortality’ within https://github.com/sinafala/svd-comp. 


# References
1. Clark, Sam, 2019, ‘A General Age-Specific Mortality Model With an Example Indexed by Child Mortality or Both Child and Adult Mortality’, Demography https://github.com/sinafala/svd-comp.

2. Gardner R. V. (2018). Sickle Cell Disease: Advances in Treatment. The Ochsner journal, 18(4), 377–389. https://doi.org/10.31486/toj.18.0076

3. www.Mortality.org (www.mortality.org/hmd/zip/all_hmd/hmd_statistics.zip)

Github Repo: https://github.com/natashagulati8/Global-Adult-and-Child-Mortality-Rates